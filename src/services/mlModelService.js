...
// Hypothetical endpoint dedicated for the Urgency Prediction page (as per spec)
const API_URL_URGENCY = 'http://127.0.0.1:5000/api/predict_urgency';
const API_URL_URGENCY_SCHEMA = 'http://127.0.0.1:5000/api/urgency/schema';
...
export default {
    predictUrgency,
    /**
     * Calls the dedicated urgency prediction API using the specified five features.
     * Expected payload shape:
     * {
     *   RequestType: string,
     *   Region: string,
     *   PopulationAffected: number,
     *   TimeSinceDisasterDays: number,
     *   ResourceAllocationIndex: number
     * }
     * Expected response: { urgency: 'Low'|'Medium'|'High', model?: string }
     */
    predictUrgencyScore: async (payload) => {
        try {
            const response = await fetch(API_URL_URGENCY, {
                method: 'POST',
            ...
            console.error('Urgency Prediction API Error:', error);
            throw error;
        }
    },
    /**
     * Fetch dynamic schema to render the input form.
     * Returns: { status: 'success', model: string, features: [{ name, type: 'number'|'select', options? }] }
     */
    getUrgencySchema: async () => {
        const response = await fetch(API_URL_URGENCY_SCHEMA);
        if (!response.ok) {
